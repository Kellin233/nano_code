...
当前位于 `capabilities/memory/`，按真实变更原因划分模块：
...
├── retrieval.py       # 查询规划、本地候选、side-query、预算打包、召回注入
└── consolidation.py   # 去重、衰减、supersede、archive、dry-run
```

`rendering.py` 已合并进 `retrieval.py`——召回记忆和格式化注入文本是同一流程的头尾，总是同时变更。
