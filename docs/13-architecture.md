...
NanoCode 的核心架构是三层单向依赖：

```
Application Layer：cli/session.py + cli/core/* + providers/* + tui/* + cli/core/server/*
Runtime Management：agent/runtime_management/*
Agent Core：agent/*
```
...
- AgentSession 是唯一装配点。
- 应用能力在 `providers/` 和 `cli/core/` 接入，并由 `AgentSession` 组合和裁剪。
- Extension 是进程内 Python 扩展，Hook 是外部进程拦截面。
- Provider adapter 物理上独立，新增厂商不改 AgentLoop。
...
| 装配点 | `AgentSession` | 产品内部 runtime | CLI runtime | Coder 对象 |
...
NanoCode 同时支持 Anthropic 和 OpenAI-compatible provider，因此不能把 Anthropic 专属能力写进 Agent Core。上下文压缩也通过注入 summary callable 避免 Runtime Management 依赖 provider。
