...
> 吸取 pi 的分层思想（依赖单向、内核不感知应用），保留 nanoCode 自身设计（Agent Core / Runtime Management / Application、Hook + Extension 互补、Python 惯例、单应用尺度）。
> 2026-06-10
...
| 原则 | 说明 |
|------|------|
| **分层单向依赖** | 三层架构，依赖箭头只向下。上层知道下层，下层不知道上层 |
...
| **Runtime Management 管运行过程** | `agent/runtime_management/` 提供 Agent 运转所需的横切工具（压缩、上下文构建、会话与运行工件持久化、权限、确认、Hook）。允许 I/O，但**不引用 cli/、tui/ 或 providers/** |
| **Application Layer 管能力和装配** | `providers/` 和 `cli/core/` 持有 provider、工具、沙箱、技能、记忆存储、MCP、扩展等能力；`cli/session.py` 是唯一装配点 |
...
| C2 | `agent/runtime_management/` 不 import `cli/` 或 `tui/` | `grep -r "from cli\|from tui" agent/runtime_management/` 有结果 |
| C3 | 依赖方向：上层依赖下层。Agent core 不 import `agent/runtime_management/`、`cli/`、`tui/`、`cli/core/`；`agent/runtime_management/` 不 import `cli/` 或 `tui/` | Agent core 文件出现上层 import，或 Runtime Management 反向 import 表现层 |
...
│  │                                                               │   │
│  │  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐        │   │
│  │  │ session  │ │ providers│ │ cli/core │ │ tui/server│       │   │
│  │  │ 装配点   │ │ 模型适配 │ │ 应用能力 │ │ 表现入口 │        │   │
│  │  └──────────┘ └──────────┘ └──────────┘ └──────────┘        │   │
│  │                                                               │   │
│  │  ┌──────────────────────────────────────────────────────┐    │   │
...
│  │                                                               │   │
│  │  ★ 允许 I/O, 不引用 cli/、tui/ 或 providers/                   │   │
│  └───────────────────────────────────────────────────────────────┘   │
│                              │                                       │
│  ┌──────────────────────────┼───────────────────────────────────┐   │
│  │                          │       Agent Core                   │   │
...
```
cli/session.py ──→ agent/runtime_management/ ──→ agent/
...
|------------|-------------------|
| 分层、依赖单向 | 上面三层，依赖箭头全部向下 |
| Agent 内核不引用扩展系统 | Agent 暴露回调槽位，应用层 AgentSession 桥接 |
| Runtime Management 作为运行过程管理层 | compressor/message_view/context/persistence/permissions/hooks/approvals 集中管理 |
...
| Pi 的运行编排器更重 | `runtime_management/` 是薄层工具集合 | nanoCode 的 agent + loop 足够简单，不需要重型编排器 |
...
└── src/                                 # package-dir 指向此处
    ...
    │   │
    │   └── runtime_management/          # Runtime Management: "怎么运转" (允许 I/O, 不引用 cli/、tui/ 或 providers/)
    │       ├── __init__.py
...
- **装配**：创建 Agent、Backend、ToolRuntime（注入 `before_tool_call` / `after_tool_call` 回调与大结果持久化 callback）、MemoryRuntime、Runtime Management 组件（compressor/context/persistence 等）、ExtensionRunner
...
这些状态由 `AgentSession` 或其装配的 runtime_management/core 组件持有。`AgentLoop` 仍只读写 Agent 状态并消费注入回调。

### 3.3 runtime_management/ 是薄层工具集合，不是重型编排器

和 Pi 的重型运行编排器不同，nanoCode 的 Runtime Management 不做编排。编排逻辑在 `agent/loop.py`（主循环状态机）和 `cli/session.py`（应用装配）。Runtime Management 只是提供**横切关注点的工具模块**：压缩、上下文构建、会话与运行工件持久化、权限、确认管理。

这符合 nanoCode 的尺度——Agent 循环本身足够简单，不需要在它外面再包一层编排器。

### 3.4 运行工件持久化

`agent/runtime_management/persistence/` 是 Runtime Management 的持久化总包，服务 Agent 运转本身，不属于 `cli/core/` 能力模块。它分两类数据：
...
这样 artifact I/O 属于 Runtime Management persistence，tool 执行管线通过窄 callback 使用它，不把 Agent core 重新耦合进来。

### 3.5 MessageView 统一工具结果读写

当前 conversation 已经是 provider-neutral 结构。`runtime_management/message_view.py` 提供一个小型读写视图，避免 compressor 在多个地方手写遍历 `ConversationMessage` / `ToolResultBlock`：
...
LLM 摘要调用不在 Runtime Management 中 import provider 实现；`cli/session.py` 根据 Backend 构造 `summarize_messages()` callable 注入 compressor。
...
Hook 覆盖"执行前拦截确认"的简单场景（30%），Extension 覆盖"给 Agent 加新能力"的复杂场景（70%）。两者都由 AgentSession 装配进运行管线：Hook 走 `agent/runtime_management/hooks/`，Extension 走 `cli/core/extensions/`，并通过 loop/ToolRuntime 回调触发，互补不冲突。
...
| 项目 | 当前实现 | 对应约束 |
|------|----------|:---:|
| 去除反向依赖 | `agent/agent.py`、`agent/loop.py`、`agent/models.py`、`agent/runtime_management/compressor.py` 不 import `tui`、`cli` 或 provider SDK。状态展示通过 `RuntimeEvent`、callback 和上层 renderer 完成 | C1, C2 |
...
| system prompt 模板 | 当前 `STABLE_SYSTEM_PROMPT` 仍在 `agent/runtime_management/context/builder.py`，并通过 `SYSTEM_PROMPT_DYNAMIC_BOUNDARY` 允许 memory 规则等动态插入 | C2 |
...
| 项目 | 当前实现 | 对应约束 |
|------|----------|:---:|
| 目录迁移 | 当前包结构已经是 `agent/`、`agent/runtime_management/`、`providers/`、`cli/`、`cli/core/`、`tui/`；旧的 `runtime`、`backend`、`capabilities` 包不在 `src/` 中 | C3 |
...
### agent/runtime_management/ (25 文件)

| 当前文件或目录 | 说明 |
|------|------|
| `src/agent/runtime_management/__init__.py` | Runtime Management public exports |
| `src/agent/runtime_management/approvals.py` | approval request/decision 管理 |
| `src/agent/runtime_management/compressor.py` | Tool History Snip / Context Compact |
| `src/agent/runtime_management/message_view.py` | canonical conversation 工具结果视图 |
| `src/agent/runtime_management/context/` | system prompt、startup context、project instructions、Git snapshot |
| `src/agent/runtime_management/hooks/` | Hook 类型、settings loader、外部命令 runner |
| `src/agent/runtime_management/permissions/` | policy、rules、shell、workspace、tool allowlist |
| `src/agent/runtime_management/persistence/` | atomic、session log/store、run store、task state、report、artifact |

细分文件：

```text
agent/runtime_management/context/{__init__.py,builder.py,sources.py}
agent/runtime_management/hooks/{__init__.py,config.py,runner.py,types.py}
agent/runtime_management/permissions/{__init__.py,policy.py,rules.py,shell.py,tool_policy.py,workspace.py}
agent/runtime_management/persistence/{__init__.py,artifacts.py,atomic.py,report.py,run_store.py,session_log.py,session_store.py,task_state.py}
```
...
| runtime compressor/approvals/context/session/artifacts | `src/agent/runtime_management/` |
