...
class AgentToolRuntimeMixin:
    ...
    def _persist_large_result(self, tool_name: str, result: str) -> str:
        ...
        if len(result.encode()) <= threshold:
            return result
        output_dir = Path.home() / ".nano-code" / "tool-results"
    ...
    async def _run_fork_skill(self, invocation: SkillInvocationResult) -> str:
        ...
        print_sub_agent_start(f"skill:{agent_type}", invocation.skill.name)
        ...
        try:
            ...
            print_sub_agent_end(f"skill:{agent_type}", invocation.skill.name)
