"""
MCP 客户端：连接基于标准输入输出的 MCP 服务器，发现并转发工具调用。
为保持简单，直接使用标准输入输出上的原始 JSON-RPC，不依赖 SDK。

配置从 .claude/settings.json 和 ~/.claude/settings.json 读取：
  { "mcpServers": { "name": { "command": "...", "args": [...], "env": {...} } } }

每个 MCP 工具都会加上 "mcp__serverName__toolName" 前缀以避免命名冲突。
"""
...
class McpConnection:
    ...
    async def initialize(self) -> None:
        ...
        await self._send_request("initialize", {
            ...
            "capabilities": {},
            "clientInfo": {"name": "nano-code", "version": "1.0.0"},
        })
...
class McpManager:
    """管理所有 MCP 服务器连接。
    先调用 load_and_connect()，再用 get_tool_definitions() 和 call_tool() 接入 Agent。"""
    ...
    def get_tool_definitions(self) -> list[dict]:
        """返回带 mcp__ 前缀、符合 Anthropic API 格式的工具定义。"""
        return [
            {
    ...
    async def call_tool(self, prefixed_name: str, args: dict) -> str:
        ...
        server_name = parts[1]
        tool_name = "__".join(parts[2:])  # 工具名本身可能包含 __。
