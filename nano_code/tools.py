"""工具定义与执行：多个工具配合 4 种权限模式。
对应 Claude Code 的工具系统：read_file、write_file、edit_file、list_files、
grep_search、run_shell、skill、agent。"""
...
ToolDef = dict  # Anthropic 工具结构定义字典
...
tool_definitions: list[ToolDef] = [
    ...
    {
        # agent 是给模型看的“派生子智能体”入口。
        # 真正执行不在本文件中，因为它需要当前 Agent 实例的模型、权限和 token 状态。
        "name": "agent",
        ...
        "input_schema": {
            ...
            "properties": {
                ...
                # type 不放进 required：模型省略时由 agent.py 回退到 general。
    ...
    if not IS_WIN:
        try:
            ...
            if result.returncode == 0:
                ...
                return output
            # 非 0/1 退出码表示异常，继续走纯 Python 兜底实现。
        except Exception:
            pass  # 继续走纯 Python 兜底实现。

    # 纯 Python 兜底实现（Windows 或系统搜索命令不可用）。
...
# ─── 执行工具调用 ────────────────────────────────────
# 为避免循环依赖，"agent" 和 "skill" 工具在 agent.py 中处理。
