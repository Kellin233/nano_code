"""记忆系统：基于文件的四类记忆，并维护 MEMORY.md 索引。
对应 Claude Code 的记忆架构：通过 sideQuery 做语义召回。"""
...
# 发送提示词并返回模型文本响应的可调用对象。
# 签名：异步函数（系统提示词，用户消息） -> 字符串
from typing import Callable
SideQueryFn = Callable[[str, str], Any]  # 实际上是可等待字符串。
...
def get_memory_dir() -> Path:
    d = Path.home() / ".nano-code" / "projects" / _project_hash() / "memory"
...
MAX_SESSION_MEMORY_BYTES = 60 * 1024  # 每个会话累计 60KB。


def scan_memory_headers() -> list[MemoryHeader]:
    """扫描记忆目录；为提高速度，只读取元数据头（前 30 行）。"""
    d = get_memory_dir()
