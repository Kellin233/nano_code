...
from .types import McpCallResult, McpDiagnostic, McpServerConfig, McpToolDef, McpToolDelta, render_resource_list
...
class McpManager:
    ...
    async def load_and_connect(self) -> None:
        ...
        for config in loaded.configs.values():
            if config.transport != "stdio":
                ...
                logger.warning("Skipping '%s': unsupported transport %s", config.name, config.transport)
                continue
    ...
    async def _connect_one(self, config: McpServerConfig) -> None:
        ...
        try:
            ...
            logger.info("Connected to '%s': %d tools", config.name, len(delta.added))
