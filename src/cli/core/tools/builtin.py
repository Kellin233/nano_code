...
BUILTIN_TOOL_DEFINITIONS: list[ToolDef] = [
    {
        ...
        "input_schema": {
            ...
            "properties": {
                ...
                "offset": {
                    "type": "integer",
                    "description": "Start reading from this line number (1-indexed). Defaults to 1.",
                },
                "limit": {
                    "type": "integer",
...
def read_file(inp: dict, *, cwd: Path | None = None) -> str:
    try:
        ...
        offset: int = max(1, int(inp.get("offset") or 1))
        limit: int | None = int(inp["limit"]) if inp.get("limit") else None
        ...
        if limit is not None and offset > 1:
            header = f"[Lines {offset}-{end} of {total}]"
        elif limit is not None:
            header = f"[Lines 1-{end} of {total}, limited to {limit}]"
        else:
