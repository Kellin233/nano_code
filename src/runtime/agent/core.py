...
本模块是 `nanocode.runtime.agent` 包的运行时核心，但它刻意不实现所有细节。
...
from ...domains.hooks import HookManager
from ...domains.context.prompt import build_prompt_bundle, build_system_prompt
from ...domains.mcp.client import McpManager
from ...domains.sandbox import SandboxConfig, SandboxManager
from ...domains.skills import ActiveSkillManager, SkillInvocation
from ...domains.tools import ToolDef, ToolRegistry, builtin_tool_definitions
from ...session import save_session
from ...tui.renderer import get_renderer


# 运行时实现已经拆到对应子模块；这里集中导出常用 helper，避免调用方
# 了解 Agent 内部文件布局。
...
class Agent(AgentContextMixin, AgentToolRuntimeMixin, AgentBackendMixin):
    ...
    `RuntimeThread` 是公开入口；子 Agent fork 和少量内部测试仍复用这个状态容器。
    """
    ...
    def _render_event(self, event: object) -> None:
        ...
        if isinstance(event, AssistantTextDelta):
            ...
            renderer.tool_call(event.call.name, event.call.input)
        elif isinstance(event, ToolCallFinished):
            renderer.tool_result(event.call.name, event.result.content)
            ...
            renderer.info(f"Budget exceeded: {event.reason}")
        elif isinstance(event, LoopFinished):
            if event.stop_reason == "stop" and not self.is_sub_agent:
                renderer.cost(self.total_input_tokens, self.total_output_tokens)
    ...
    def restore_session(self, data: dict) -> None:
        ...
        get_renderer().info(f"Session restored ({self._get_message_count()} messages).")
    ...
    def _handle_mcp_tool_delta(self, delta, definitions: list[dict]) -> None:
        ...
        try:
            from ...domains.context.attachments import render_mcp_delta_attachment
