...
def check_path_policy(tool_name: str, inp: dict, cwd: Path | None = None) -> PathDecision:
    ...
    if _is_protected(path, workspace):
        if tool_name in {"write_file", "edit_file"}:
            return PathDecision("confirm", f"protected path: {path}")
        if tool_name == "read_file":
            return PathDecision("confirm", f"read protected path: {path}")

    if not _is_relative_to(path, workspace):
        return PathDecision("confirm", f"path outside workspace: {path}")
